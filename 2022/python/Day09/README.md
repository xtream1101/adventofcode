...
by following a hypothetical _series of motions_ (your puzzle input) for the
...
This series of motions moves the head _right_ four steps, then _up_ four
steps, then _left_ three steps, then _down_ one step, and so on. After each
...
Simulate your complete hypothetical series of motions. _How many positions
does the tail of the rope visit at least once?_
...
this example, the tail never moves, and so it only visits _`1`_ position.
However, _be careful_ : more types of motion are possible than before, so you
