...
for guard in guard_times.keys():
    minute = max(guard_times[guard]['minutes_asleep'],
                 key=guard_times[guard]['minutes_asleep'].get)
    count = guard_times[guard]['minutes_asleep'][minute]
    if result is None:
        result = (guard, minute, count)
    else:
        if result[2] < count:
            result = (guard, minute, count)

print(f"Part 2: {int(result[0]) * result[1]}")
